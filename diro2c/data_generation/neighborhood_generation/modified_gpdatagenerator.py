...
def fitness_sso(x0, bb1, bb2, diff_classifier_method, alpha1, alpha2, eta, discrete, continuous, class_name, idx_features, distance_function, x1):
    ...
    sim_ratio = 1.0 - \
        distance_function(x0d, x1d, discrete, continuous, class_name)
    record_similarity = 0.0 if sim_ratio >= eta else sim_ratio

    y0 = bb1.predict(np.asarray(x1).reshape(
        1, -1))[0]
    y1 = bb2.predict(np.asarray(x1).reshape(
        1, -1))[0]
...
def fitness_sdo(x0, bb1, bb2, diff_classifier_method, alpha1, alpha2, eta, discrete, continuous, class_name, idx_features, distance_function, x1):
...
def fitness_dso(x0, bb1, bb2, alpha1, alpha2, eta, discrete, continuous, class_name, idx_features, distance_function, x1):
...
def fitness_ddo(x0, bb1, bb2, alpha1, alpha2, eta, discrete, continuous, class_name, idx_features, distance_function, x1):
...
def setup_toolbox(record, feature_values, bb1, bb2, diff_classifier_method, init, init_params, evaluate, discrete, continuous, class_name,
    ...
    toolbox.register("individual", tools.initIterate,
                     creator.individual, toolbox.feature_values)
    toolbox.register("population", tools.initRepeat, list,
                     toolbox.individual, n=population_size)

    toolbox.register("clone", cPickle_clone)
    toolbox.register("evaluate", evaluate, record, bb1, bb2, diff_classifier_method, alpha1, alpha2, eta, discrete, continuous,
...
def get_oversample(population, halloffame):
    ...
    fitness_diff = [fitness_values[i+1] - fitness_values[i]
                    for i in range(0, len(fitness_values)-1)]

    index = np.max(np.argwhere(fitness_diff == np.amax(
        fitness_diff)).flatten().tolist())
...
def generate_modified_data(x, feature_values, bb1, bb2, diff_classifier_method, discrete, continuous, class_name, idx_features, distance_function,
                           neigtype='all', population_size=1000, halloffame_ratio=0.1, alpha1=0.5, alpha2=0.5, eta1=1.0,
                           eta2=0.0, tournsize=3, cxpb=0.5, mutpb=0.2, ngen=10, return_logbook=False, max_steps=1, is_unique=False):
    ...
    while lenUniqueXgp < population_size and i <= max_steps:

        if size_sso > 0.0:
            toolbox_sso = setup_toolbox(x, feature_values, bb1, bb2, diff_classifier_method, init=record_init, init_params=x, evaluate=fitness_sso,
                                        ...
                                        population_size=size_sso*i, alpha1=alpha1, alpha2=alpha2, eta=eta1, mutpb=mutpb,
                                        tournsize=tournsize)
        ...
        if size_sdo > 0.0:
            toolbox_sdo = setup_toolbox(x, feature_values, bb1, bb2, diff_classifier_method, init=record_init, init_params=x, evaluate=fitness_sdo,
                                        discrete=discrete, continuous=continuous, class_name=class_name,
                                        idx_features=idx_features, distance_function=distance_function,
                                        population_size=size_sdo*i, alpha1=alpha1, alpha2=alpha2, eta=eta1, mutpb=mutpb,
                                        tournsize=tournsize)
        ...
        if size_dso > 0.0:
            toolbox_dso = setup_toolbox(x, feature_values, bb1, bb2, diff_classifier_method, init=record_init, init_params=x, evaluate=fitness_dso,
                                        discrete=discrete, continuous=continuous, class_name=class_name,
                                        idx_features=idx_features, distance_function=distance_function,
                                        population_size=size_dso*i, alpha1=alpha1, alpha2=alpha2, eta=eta2, mutpb=mutpb,
                                        tournsize=tournsize)
        ...
        if size_ddo > 0.0:
            toolbox_ddo = setup_toolbox(x, feature_values, bb1, bb2, diff_classifier_method, init=record_init, init_params=x, evaluate=fitness_ddo,
                                        discrete=discrete, continuous=continuous, class_name=class_name,
                                        idx_features=idx_features, distance_function=distance_function,
                                        population_size=size_ddo*i, alpha1=alpha1, alpha2=alpha2, eta=eta2, mutpb=mutpb,
                                        tournsize=tournsize)
        ...
        if is_unique:
            lenUniqueXgp = len(
                np.unique((np.concatenate((Xgp), axis=0)), axis=0))
        i += 1
