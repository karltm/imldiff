...
def main():
    ...
    fig = plot_decision_regions(X=X1.astype(np.int64), y=y1.astype(
    ...
    fig = plot_decision_regions(X=X2.astype(np.int64), y=y2.astype(
    ...
    fig = plot_decision_regions(X=X_diff.astype(np.int64), y=y_diff.astype(
        np.int64), clf=dc_full, ax=ax[2], legend=2, X_highlight=np.asarray([X1[20]]))
